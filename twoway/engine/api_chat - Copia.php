<?php
// api_chat.php - Versão com Upload de Mídia e Respostas
session_start();
include 'config.php';
...
// ------------------------------------------
    ...
    case 'send_media':
        ...
        // 1. Caminho físico (onde o PHP vai gravar o arquivo agora)
        $destination = $upload_dir . $filename;

        if (move_uploaded_file($file['tmp_name'], $destination)) {
            
            // 2. CAMINHO PARA O BANCO (como o chat.php verá o arquivo depois)
            // Como o chat.php está na mesma pasta que a /uploads, o link deve ser apenas:
            $path_for_db = 'uploads/' . $filename; 
            
            // Note que aqui mudamos $destination para $path_for_db
            $stmt = $conn->prepare("INSERT INTO mensagens (remetente_id, destinatario_id, mensagem, arquivo_path, reply_to_id) VALUES (?, ?, ?, ?, ?)");

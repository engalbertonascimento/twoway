    ...
    <div id="sidebar">
        ...
        <div id="out-config">
            ...
            <?php if (isset($_SESSION['nivel_acesso']) && $_SESSION['nivel_acesso'] == 'admin'): ?>
                <a href="../../admin/index.php" title="Painel de Controle" style="text-decoration:none; font-size: 14px;">⚙️</a>
            <?php endif; ?>
        </div>
...
<script>
    ...
    // Objeto para controlar o ID da última mensagem de cada contato (Notificação Global)
    let historicoGlobal = {};
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        // Inicia a vigilância de outros contatos após carregar a página
        setInterval(vigiarOutrosContatos, 4000); 
    });
    ...
    // --- NOVA FUNÇÃO: VIGIA MENSAGENS DE QUEM NÃO ESTÁ SELECIONADO ---
    function vigiarOutrosContatos() {
        const contatos = document.querySelectorAll('.contact-item');
        contatos.forEach(contato => {
            const id = contato.dataset.id;
            const nome = contato.dataset.username;

            // Se for o chat aberto, o loadMessages já cuida. Se não, vigiamos:
            if (id !== destinatarioId) {
                fetch(`../../engine/api_chat.php?action=get_messages&destinatario_id=${id}`)
                    .then(res => res.json())
                    .then(data => {
                        if (data.length > 0) {
                            const ultima = data[data.length - 1];
                            const idMensagem = parseInt(ultima.id);

                            // Se temos um histórico e a mensagem atual é mais nova que a última registrada
                            if (historicoGlobal[id] && idMensagem > historicoGlobal[id]) {
                                // Se não fui eu quem enviou
                                if (parseInt(ultima.remetente_id) !== parseInt(REMETENTE_ID)) {
                                    dispararNotificacao(nome, ultima.mensagem);
                                }
                            }
                            // Atualiza o histórico desse contato
                            historicoGlobal[id] = idMensagem;
                        }
                    }).catch(err => console.error("Erro na vigilância:", err));
            }
        });
    }
        ...
        fetch(`../../engine/api_chat.php?action=get_messages&destinatario_id=${destinatarioId}`)
            ...
            .then(data => {
                if (data.length > 0) {
                    ...
                    // Sincroniza o histórico global para evitar notificação duplicada ao abrir o chat
                    historicoGlobal[destinatarioId] = parseInt(ultimaMsg.id);
